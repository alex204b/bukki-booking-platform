...
async function applyMigration() {
  ...
  const dataSource = new DataSource(
    dbUrl
      ? { type: 'postgres', url: dbUrl, ssl: useSSL }
      : {
          type: 'postgres',
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../migrations/019-add-business-type-and-assignment.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');

    console.log('Applying migration 019: Add businessType and bookingAssignment...');
